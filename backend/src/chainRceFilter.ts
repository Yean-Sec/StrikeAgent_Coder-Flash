/**
 * 组合链入库/展示口径：仅 none|user → 终点为真实 RCE/完全接管。
 * 与 vuln-chain-exploiter 技能一致；勿用 detail 里的作业 ID（如 pt-rce）误判。
 */
...
function chainLabel(c: ChainLike): string {
  return String(c.name ?? c.chain_name ?? c.chain ?? '').trim();
}
...
export function isRceTerminalChain(c: ChainLike | null | undefined): boolean {
  ...
  const name = chainLabel(c);
  const pathHint = String(c.chain_path ?? '').trim();
  const focus = `${impact} ${name} ${pathHint}`.toLowerCase();
...
export function isAllowedChainAuth(c: ChainLike | null | undefined): boolean {
  ...
  const text = `${chainLabel(c as ChainLike)} ${c?.impact ?? ''}`.toLowerCase();
